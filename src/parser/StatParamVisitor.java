...
//		this.partialTokens.append(" null ");
	...
	public String getS(ASTNode node){
		if(node instanceof  SimpleName){
			...
			if (obj.getExpression() != null && obj.getExpression() instanceof TypeLiteral) {
				TypeLiteral lit = (TypeLiteral) obj.getExpression();
				String utype = getUnresolvedType(lit.getType()), rtype = getResolvedType(lit.getType());
//				this.fullTokens.append(" " + rtype + ".class." + obj.getName().getIdentifier() + "() ");
//				this.partialTokens.append(" " + utype + ".class." + obj.getName().getIdentifier() + "() ");
			} else {
...
//				((ASTNode) node.arguments().get(i)).accept(this);
	...
	public String abstractSimpleName(SimpleName node){
		//return "";
		IBinding b = node.resolveBinding();
		if (b != null) {
			if (b instanceof IVariableBinding) {
